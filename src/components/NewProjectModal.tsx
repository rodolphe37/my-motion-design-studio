...
export function NewProjectModal({ open, onClose }: Props) {
  ...
  const [name, setName] = useState('Projet sans titre');
  ...
  const steps = ['Nom', 'Mode', 'Format', 'Réglages'];
      ...
      <div className="relative panel w-full max-w-2xl max-h-[90vh] overflow-y-auto scrollbar-thin animate-scale-in shadow-2xl">
        ...
        <div className="p-5 space-y-4">
          {step === 0 && (
            <div className="space-y-3">
              ...
              <input
                ...
                autoFocus
              />
              <p className="text-sm text-ink-400">Vous pourrez le renommer plus tard dans l'éditeur.</p>
            </div>
          )}

          {step === 1 && (
            <div className="space-y-3">
              <label className="label">Mode du projet</label>
              <div className="grid grid-cols-2 gap-3">
                ...
                >
                  ...
                  <h3 className="font-semibold mb-1">2D</h3>
                  <p className="text-sm text-ink-300">Scène plane, formes vectorielles, texte, images. Léger et rapide, idéal réseaux sociaux.</p>
                </button>
                <button
                  onClick={() => setMode('3d')}
                  className={`relative p-5 rounded-xl border-2 text-left transition-all ${
                ...
                >
                  ...
                  <p className="text-sm text-ink-300">Scène volumétrique, objets, caméra et éclairage. Idéal logo reveal et showreels.</p>
                </button>
              </div>
              <p className="text-sm text-amber-400/80 flex items-center gap-1.5">
                <Check className="w-4 h-4" />
                Ce choix est définitif. Pour changer, dupliquez le projet.
              </p>
            </div>
          )}
          ...
          {step === 2 && (
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                ...
                >
                  <div className="flex items-center justify-center mb-2 h-12 text-ink-300">Custom</div>
                  <div className="text-xs font-medium">Personnalisé</div>
                </button>
              </div>
              {preset === 'custom' && (
                <div className="flex items-end gap-2 p-3 bg-ink-900 rounded-lg border border-ink-600">
                  ...
                  <button onClick={() => setRatioLocked(!ratioLocked)} className="icon-btn mb-0.5" title="Verrouiller le ratio">
          ...
          {step === 3 && (
            <div className="space-y-4">
                ...
                <>
                  ...
                  <div className="flex items-center justify-between p-3 bg-ink-900 rounded-lg border border-ink-600">
                    <div>
                      ...
                      <div className="text-xs text-ink-400">Activer les ombres par défaut</div>
                    </div>
              ...
              <div className="space-y-2">
                <label className="label">Durée par défaut d'une scène (secondes)</label>
        ...
        <div className="flex items-center justify-between p-5 border-t border-ink-700 sticky bottom-0 bg-ink-850">
          <button
            ...
            className="btn-ghost"
          >
            {step === 0 ? 'Annuler' : <><ArrowLeft className="w-4 h-4" /> Précédent</>}
          </button>
          {step < 3 ? (
            <button
